...
```bash
python main_app.py                                    # Run desktop app
python server.py --host 0.0.0.0 --port 5000           # Run LAN server
pyinstaller "TCL表格比对.spec" --clean --noconfirm     # Build EXE (Win10+)
pyinstaller build_win7.spec --clean --noconfirm        # Build EXE (Win7)
python setup_cx_freeze.py build                        # Build for Windows 7 (cx_Freeze, alternative)
```

No virtual environment or formal test suite. Ad-hoc test scripts exist (`test_*.py`) but no pytest/unittest framework.
...
- **Local mode**: JSON cache files (`db_cache.json`, `batch_import_cache.json`, `output_dir_config.json`)
...
- `test_*.py` files — ad-hoc test scripts, not a formal test suite
- 3 `.spec` files: `TCL表格比对.spec` (main), `build_win7.spec`, `build_win7_fixed.spec`
- `runtime_fix_win7.py`, `win7_launcher.py` — Win7 compatibility helpers, used by cx_Freeze build
